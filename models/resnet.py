...
def resnet101_mtl(num_expr_classes, num_au_classes, pretrained=False):
    net = ResNet(block=Bottleneck, layers=[3, 4, 23, 3])
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls["resnet101"])
    ...
    net.expr_decoder = MLDecoder(
        num_classes=num_expr_classes,
        num_of_groups=num_expr_classes,
        decoder_embedding=768,
        initial_num_features=2048  
    )

    net.au_decoder = MLDecoder(
        num_classes=num_au_classes,
        num_of_groups=num_au_classes,
        decoder_embedding=768,
        initial_num_features=2048
    )

    def forward_with_au(x, le=False, ft=False):
        x_ = net.conv1(x)
        x_ = net.bn1(x_)
        x_ = net.relu(x_)
        x_ = net.maxpool(x_)
        ...
        if le:
            le_expr, expr_logits = net.expr_decoder(feat, le=True)
            le_au, au_logits = net.au_decoder(feat, le=True)
            return feat, (le_expr, le_au), (expr_logits, au_logits)
        else:
            expr_logits = net.expr_decoder(feat)
            au_logits = net.au_decoder(feat)
            return expr_logits, au_logits

    net.forward = forward_with_au

    return net
def resnet34_mtl(num_expr_classes, num_au_classes, pretrained=False):
    net = ResNet(block=BasicBlock, layers=[3, 4, 6, 3])
    ...
    net.proj_to_decoder = nn.Conv2d(512, 2048, kernel_size=1)
